...
for thick in "${THICK_VALUES[@]}"; do
  ...
  {
    echo ">>> Starting TEMPLATE-MCMC TEST for $oid thick=$thick at $(date)"
    ...
    if [ "$ec" -eq 0 ]; then
      echo
      echo "<<< Finished TEMPLATE-MCMC TEST $oid thick=$thick at $(date)"
    else
      echo
      echo "!!! FAILED TEMPLATE-MCMC TEST $oid thick=$thick exit=$ec at $(date)"
    fi
  } > "$logfile" 2>&1
